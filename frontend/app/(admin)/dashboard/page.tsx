...
export default function DashboardPage() {
  // fullName and user come from useAuth — no NextAuth type dependency
    ...
    <div className="space-y-6 max-w-7xl">
      <div className="bg-green-800 rounded-2xl px-6 py-5 text-white flex items-center justify-between">
        <div>
          ...
          <h2 className="font-bold text-2xl mt-0.5">{displayName} 👋</h2>
        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 bg-white rounded-xl border border-gray-200 shadow-sm p-5">
              ...
              <p className="text-xs text-gray-400 mb-5">Lunas vs Belum Bayar</p>
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
              ...
              <div className="divide-y divide-gray-100 max-h-[240px] overflow-y-auto">
                {unpaid.length === 0 ? (
                  <div className="px-5 py-8 text-center text-sm text-gray-400">🎉 Semua warga sudah bayar!</div>
                ) : unpaid.slice(0, 6).map((inv) => (
                  <div key={inv.id} className="px-5 py-3 flex items-center gap-3 hover:bg-gray-50">
                    <Avatar name={inv.resident_name ?? "W"} size="sm" />
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-medium text-gray-800 truncate">{inv.resident_name ?? "Warga"}</p>
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
              ...
              <button className="text-xs text-green-600 flex items-center gap-1 font-medium">
                Lihat semua <ChevronRight className="w-3 h-3" />
              </button>
            </div>
            <div className="divide-y divide-gray-100">
              {warga.length === 0 ? (
                <div className="px-5 py-10 text-center text-sm text-gray-400">Belum ada warga terdaftar</div>
              ) : warga.slice(0, 5).map((r) => (
                <div key={r.id} className="px-5 py-3.5 flex items-center gap-4 hover:bg-gray-50">
